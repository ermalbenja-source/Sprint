...
-- ── kodi i gjurmimit ───────────────────────────────────────────────────────
-- Tetë shenja pa shkronjat dhe shifrat që ngatërrohen (pa I, O, 0, 1), që
-- klienti ta shkruajë ose ta lexojë në telefon pa gabime: p.sh. A4F7-K2M9.
...
create table if not exists public.orders (
  ...
  token         text        not null default public.gen_order_code(),
...
-- Për instalimet e vjetra: kolona e kodit dhe defaulti i ri.
alter table public.orders add column if not exists token text;
alter table public.orders alter column token set default public.gen_order_code();
update public.orders set token = public.gen_order_code() where token is null;

create index if not exists orders_created_idx on public.orders (created_at desc);
create index if not exists orders_status_idx  on public.orders (status);
create unique index if not exists orders_token_idx on public.orders (token);
create index if not exists orders_token_upper_idx on public.orders (upper(token));
...
returns table (
  code text, number int, status text, kind text, total numeric,
...
as $$
  select o.token, o.number, o.status, o.kind, o.total,
         o.created_at, o.accepted_at, o.prep_minutes, o.items
  from public.orders o
  where upper(o.token) = upper(regexp_replace(coalesce(p_token, ''), '[^A-Za-z0-9]', '', 'g'))
    and length(regexp_replace(coalesce(p_token, ''), '[^A-Za-z0-9]', '', 'g')) >= 6
  limit 1;
$$;

-- ── kërkimi me numër telefoni ──────────────────────────────────────────────
-- Kthen VETËM porositë ende në punë të 24 orëve të fundit, dhe vetëm fusha
-- jo-personale. Porositë e mbyllura nuk dalin fare, që historia e askujt të
-- mos jetë e kërkueshme me numër telefoni.
create or replace function public.find_orders_by_phone(p_phone text)
returns table (
  code text, number int, status text, kind text, total numeric,
  created_at timestamptz, accepted_at timestamptz, prep_minutes int
)
language sql
security definer
stable
set search_path = public
as $$
  with q as (select regexp_replace(coalesce(p_phone, ''), '\D', '', 'g') as digits)
  select o.token, o.number, o.status, o.kind, o.total,
         o.created_at, o.accepted_at, o.prep_minutes
  from public.orders o, q
  where length(q.digits) >= 6
    and right(regexp_replace(o.phone, '\D', '', 'g'), 9) = right(q.digits, 9)
...
revoke all on function public.find_orders_by_phone(text) from public;
grant execute on function public.find_orders_by_phone(text) to anon, authenticated;
